...
class Node:
    ...
    def __repr__(self):
        return '{val}{color} '.format(color=self.color, val=self.value)
...
start = False
i, o, e = select.select([sys.stdin], [], [], 0.97-(time.time()-start_time))
if i:
    start = True
com = []
while com or start:
    ...
    if com:
        if com[0] == "insert":
            rb_tree.add(int(com[1]))

        elif com[0] == "print":
            print(rb_tree.BFS())

        elif com[0] == "delete":
            rb_tree.remove(int(com[1]))
    else:
        break
